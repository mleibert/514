...
b.prop( x = udat$x , y = udat$y , yhat= 1 , b=b0 ,w= uw0 ,neg.logll=T)


#multivariate
num.gradient(cost,x = mdat$x ,mdat$y,b0 , mw0  ,h = .00001,neg.logll=T )
b.prop( x = mdat$x , y = mdat$y , yhat= 1 , b=b0 ,w= mw0 ,neg.logll=T)
...
plot( fit$W, fit$B , xlab = "performance?" )

par(mfrow=c(1,1) , mai= c(.85 , .5, .5 , .5 ) )
z <- cbind( fit$W, fit$B , fit$cost)

contour(x = seq(-1,  10, length.out = nrow(z)),
        y = seq(-1,  1, length.out = ncol(z)),  z , nlevels = 50)
...
c( fit$b, fit$w )
coef( glm( DF$y  ~ DF$x , family=gaussian ) ) # compare Fisher's scoring 
...
contour(x = seq(-1,  6, length.out = nrow(z)),
        y = seq(-1,  1, length.out = ncol(z)),  z , nlevels = 50)
lines( fit$W , col = "red", type = "o", pch = "16")
```
...
z <- cbind( wgt_history[[4]][[1]] , wgt_history[[4]][[2]] , cost_history[[4]])
par(mfrow=c(1,1) , mai= c(.85 , .5, .5 , .5 ) )
contour(x = seq(-1,  3*18, length.out = nrow(z)),
        y = seq(-0,  6, length.out = ncol(z)),  z , nlevels = 50)
lines( wgt_history[[4]]$w , col = "red", type = "o", pch = "16")
