...
minpix <- pix0[which( pix0[,no + 1 ] == min(pix0[,no + 1 ] ) ), ncol(pix0)]
maxpix <- pix0[which( pix0[,no + 1 ] == max(pix0[,no + 1 ] ) ), ncol(pix0)]

max( fP[,minpix] )
which( fP[,minpix]  == max( fP[,minpix] ) )-1
show_digit2(x[,minpix])
show_digit2(x[,maxpix])
