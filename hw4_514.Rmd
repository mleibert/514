...
classes <- sort(unique(as.vector(y)))
		...
		sum( (classes[apply( fP , 2 , function( M ) which( M == max(M) ))] == y ) *1  ) / length(y)
...
fP <- fwd.prop(x , fit$b, fit$W )
dim(fP)
pix <- cbind( t(fP) , as.matrix(y) , 1:nrow(fP))
head(pix)
 
pix0 <- pix[which(pix[,11] == 0),]
head( pix0[ which( pix0[,1] == min( pix0[,1] ) ) ,]  )
show_digit2(x[ , 8 ])
y[7]
