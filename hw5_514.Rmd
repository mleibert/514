...
```{r, cache = T}
fit.LG1 <- nnet1.fit( x, y, 1 , 2 , 150000, 1.5,   tanh,  Identity );  #beep("coin") 
```


```{r LG2, cache = T }
fit.LG2 <- nnet1.fit( x, y, 1 , 5 , 150000, 1.5,   tanh,  Identity );  #beep("coin") 
```

```{r LG3, cache = T }
fit.LG3 <- nnet1.fit( x, y, 1 , 100 , 150000, 1.5,   tanh,  Identity );  #beep("coin") 
```
...
plot(x.set, y.set, type='p', col='blue', 	pch=19, ylim=c(-3,3), main='Linear Fit by N.Hid')
...
```{r SP1, cache= T }
fit.sp1 <- nnet1.fit( x, y, 1 , 5 , 100000,  .5,   tanh,  Sigmoid );  #beep("coin") 
```


```{r SP2, cache= T }
fit.sp2 <- nnet1.fit( x, y, 1 , 5 , 100000,  .5,   relu,  Sigmoid );  #beep("coin") 
```

```{r, cache= T }
fit.sp3 <- nnet1.fit( x, y, 1 , 5 , 100000, 3,   sigmoid,  Sigmoid );  #beep("coin") 
```

```{r, cache= T }
fit.sp4 <- nnet1.fit( x, y, 1 , 100 , 100000,  .5,   tanh,  Sigmoid );  #beep("coin") 
```


```{r, cache= T }
fit.sp5 <- nnet1.fit( x, y, 1 , 100 , 100000,  .5,   relu,  Sigmoid );  #beep("coin") 
```

```{r, cache= T }
fit.sp6 <- nnet1.fit( x, y, 1 , 100 , 100000, 3,   sigmoid,  Sigmoid );  #beep("coin") 
```


```{r }
spfit.pred1 <- ifelse( fit.sp1$yhat  > .5 , 1 , 0    )
sum(( y  ==   as.numeric( spfit.pred1  ) ) * 1 ) / length(y)
...
sum(( y  ==   as.numeric( spfit.pred4  ) ) * 1 ) / length(y)
...
```{r spplots, echo=F, fig.height = 9, fig.width = 6, fig.align = "center"}
...
```{r, cache = T}
mnist.fit <- nnet1.fit( x, y, 1 , 30 , 50,  .15,   relu,  stable.softmax );  
```
...
fit.sp1.b <- nnet1.fit.batch( x  , y , 1 , 3 ,  100 , 1500, 3,	Activation = tanh, Output = Sigmoid, 32 )
```

```{r batch.sigmoid.2, cache=T }
fit.sp2.b <- nnet1.fit.batch( x  , y , 1 , 3 ,  100 , 1500, 3,	Activation = sigmoid, Output = Sigmoid, 64 )
```

```{r, eval= T}
fit.sp1.b$iter
plot(fit.sp1.b$performance, type="l")
#plot(fit.sp1.b$costs, type="l" )
```

```{r batch.relu, cache=T, eval= F }
...
```{r, cache = T }
mnist.fit.bat <- nnet1.fit.batch( x  , y , 1 , 2000 ,  100 , 10, .15,	Activation = relu, Output = stable.softmax )
```

```{r, cache=T}
yhat <- fwd.prop( x  , 1 , mnist.fit.bat$W,  mnist.fit.bat$B, relu, stable.softmax)
yhat <- yhat$A[[2]]
