...
We will use fine foods reviews to predict user sentiment toward products reviewed on Amazon. The sentiment is measured on a scale of 1  to 5, with 5 being the highest positive sentiment. Our dataset contains both short review summary, as well as the full length review. We will start by focusing on predicting user sentiment from the short review summaries, then we will attempt to scale up and use the full reviews for prediction. The dataset also contains other variables, such as user ID, product ID, and the ``helpfulness'' of the review (number of users that rated the review helpful and the number of users that rated the reviews), that we may use in constructing the model.
